    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                print(f"I2C read error (attempt {attempt+1}/{retries}): {e}")
...
class AWSIoTManager:
    ...
    def publish_door_state(self, state, confidence=None, timestamp=None):
        """
        Publish door state event to AWS IoT
        
        Args:
            state: Door state ('Open' or 'Close')
    ...
    def publish_classification_result(self, result_data):
        ...
        try:
            ...
            # Use a results topic if available, otherwise just return
...
class DoorStateDetector:
    def __init__(self, model_path=MODEL_PATH, scaler_path=SCALER_PATH, aws_manager=None):
            ...
            if len(self.data_buffer) >= WINDOW_SIZE:
                ...
                if current_time - self.last_classification_time > self.min_time_between_classifications:
                    ...
                    if class_name:
                        ...
                        # Publish to AWS IoT
                        if self.aws_manager:
                            self.aws_manager.publish_door_state(
                                state=class_name,
...
def main():
    ...
    aws_manager = AWSIoTManager()
    
    # Initialize detector
    detector = DoorStateDetector(aws_manager=aws_manager)
    ...
    try:
        ...
        if aws_manager.connected:
            print(f"AWS IoT Publishes: {aws_manager.publish_count}")
            print(f"AWS IoT Errors: {aws_manager.publish_errors}")
        print("="*70 + "\n")
